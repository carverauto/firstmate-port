...
- `fm-steer` is the captain CLI. Device-code against this API. It must not import or dial NATS. New CLI surface is thin HTTP only: no ranking, quota math, or provider keys in Go. Captain-facing usage, including the standing prompt that makes stock firstmate use portal steering without a fork, is `docs/fm-steer.md`.
