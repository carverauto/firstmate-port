...
- The portal hostname is public. `/terms`, `/privacy`, `/healthz` and `/login` are the only routes that answer signed out. Public-edge posture — client-IP resolution, rate-limit buckets, sign-in lockout, CSP presets — is `docs/security.md`; read it before adding a public route or changing a router pipeline. MCP and NATS get no hostname of their own.
