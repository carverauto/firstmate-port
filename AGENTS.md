...
- Tenant secrets are rows, not cluster secrets: tenants enter them in the portal, `AshCloak` encrypts them into CNPG, and only `CLOAK_KEY` stays in Kubernetes. Never add a per-tenant `kubectl create secret`, and never expose plaintext over HTTP or MCP. See `docs/credentials.md`.
