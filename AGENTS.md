...
- `fm-steer` is the captain CLI. Device-code against this API. It must not import or dial NATS. New CLI surface is thin HTTP only: no ranking, quota math, or provider keys in Go. Its default instance is the public portal; local work passes `--instance` or `FIRSTMATE_INSTANCE`. Captain-facing usage, including the standing prompt that makes stock firstmate mirror steers here without a fork, is `docs/fm-steer.md`.
...
- The fleet log is append-only: an agent that "edits" progress POSTs a new event and never `UPDATE`s a historical row, and the UI is a projection over those events. Contract and scope in `docs/fleet-log.md`; not yet implemented.
