...
- `fm-steer` is the captain CLI. Device-code against this API. It must not import or dial NATS. New CLI surface is thin HTTP only: no ranking, quota math, or provider keys in Go. Captain-facing CLI usage is `docs/fm-steer.md`; the importable standing prompt is owned by `integrations/firstmate/` (see below).
