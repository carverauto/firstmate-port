defmodule FirstmatePortWeb.FleetLogJourneyTest do
  ...
  test "scheduled GitHub ingestion and agent posts populate the signed-in Fleet log", %{
    conn: conn
  } do
    ...
    for {path, payload} <- [
      ...
      response =
        build_conn() |> put_req_header("authorization", "Bearer " <> token) |> post(path, payload)
