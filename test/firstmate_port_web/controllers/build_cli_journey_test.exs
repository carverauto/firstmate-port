defmodule FirstmatePortWeb.BuildCliJourneyTest do
  ...
  test "crew reports docker, k8s and other systems through fm-steer and HTTP", %{tmp_dir: tmp_dir} do
    ...
    {output, code} =
      System.cmd("go", ["build", "-o", binary, "./cmd/fm-steer"], stderr_to_stdout: true)
    ...
    server =
      start_supervised!({Bandit, plug: FirstmatePortWeb.Endpoint, ip: {127, 0, 0, 1}, port: 0})
    ...
    transcript =
      Enum.map_join(
        [{"docker", "success"}, {"k8s", "failure"}, {"bazel", "cancelled"}],
        "\n",
        fn {kind, status} ->
          start_args = [
            "build",
          ...
          assert %{"run_id" => ^run_id, "status" => ^status, "kind" => ^kind} =
                   Jason.decode!(finish_output)

          response =
            Req.get!(instance <> "/api/build-events",
    ...
    if path = System.get_env("BUILD_JOURNEY_EVIDENCE") do
      File.write!(
        path,
        "Actual fm-steer binary → HTTP API → isolated PostgreSQL.\n" <>
