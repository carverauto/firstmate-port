// firstmatectl talks only to the firstmate-port HTTP API.
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: firstmatectl auth login|status|logout | inbox put|next|ack|list\n")
	os.Exit(2)
}
...
func authLogin(args []string) {
	...
	if err := postJSON(base+"/api/cli/auth/device", "", map[string]string{"client_id": "firstmatectl"}, &issued); err != nil {
		log.Fatal(err)
	}
...
func credsDir() string {
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "firstmatectl")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "firstmatectl")
}
...
func mustCreds(instance string) creds {
	c, err := readCreds()
	if err != nil || c.Token == "" {
		log.Fatal("not logged in; run firstmatectl auth login")
	}
	if instance != "" {
