defmodule FirstmatePort.Jobs.DiscordFanout do
  ...
  defp render("diagram", id, _tenant) do
    "**Archify diagram** #{public_url()}/d/#{id}"
  end

  defp render("progress", id, _tenant) do
    "**Progress** #{public_url()}/?tab=progress##{id}"
  end

  defp render("roll", id, _tenant) do
    "**Roll** #{public_url()}/rolls/#{id}"
  end

  defp render("no_mistakes", id, tenant) do
    # Generic on every public surface. Findings stay on the LAN portal.
    "**no-mistakes** #{generic_nm(id, tenant)} #{public_url()}/no-mistakes"
  end

  defp render(_, _, _), do: nil

  defp generic_nm(id, tenant) do
